...
test_main(void)
{
	...
	/* post condition */
	CU_ASSERT_EQUAL(test_bitmap_isr, 0x55);
}

// ISR
void isr_15(intptr_t exinf)
{
	CU_ASSERT_EQUAL(exinf, EXINF1); // 引数が正しく渡されることの確認
	CU_PASS("test_attisr_102: isr_15");	// ISRへ到達したことの確認
...
{
	CU_PASS("test_attisr_102: i_begin_int");	// ISRへ到達したことの確認
}
...
{
	CU_PASS("test_attisr_102: i_end_int");	// ISRへ到達したことの確認
}
