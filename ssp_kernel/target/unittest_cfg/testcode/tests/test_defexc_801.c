...
extern void _kernel_exchdr_5_5(void);
...
{
	// CPU例外
	CU_ASSERT_EQUAL(tnum_excno, 8); /* TNUM_EXCNO が直接テスト出来ないので代わりにこれだけテストする */
	...
	CU_ASSERT_EQUAL(excinib_excno[3], 5);
	CU_ASSERT_EQUAL(excinib_excatr[3], TA_NULL);
	CU_ASSERT_EQUAL(excinib_entry[3], (FP)(EXC_ENTRY(5, exchdr_5)));
	
	CU_ASSERT_EQUAL(excinib_excno[4], 6);
	CU_ASSERT_EQUAL(excinib_excatr[4], TA_NULL);
	CU_ASSERT_EQUAL(excinib_entry[4], (FP)(EXC_ENTRY(6, exchdr_6)));
	
	CU_ASSERT_EQUAL(excinib_excno[5], 11);
	CU_ASSERT_EQUAL(excinib_excatr[5], TA_NULL);
	CU_ASSERT_EQUAL(excinib_entry[5], (FP)(EXC_ENTRY(11, exchdr_11)));
	
	CU_ASSERT_EQUAL(excinib_excno[6], 12);
	CU_ASSERT_EQUAL(excinib_excatr[6], TA_NULL);
	CU_ASSERT_EQUAL(excinib_entry[6], (FP)(EXC_ENTRY(12, exchdr_12)));
	
	CU_ASSERT_EQUAL(excinib_excno[7], 14);
	CU_ASSERT_EQUAL(excinib_excatr[7], TA_NULL);
	CU_ASSERT_EQUAL(excinib_entry[7], (FP)(EXC_ENTRY(14, exchdr_14)));
